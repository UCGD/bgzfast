...
sub EXISTS {
    ...
    $self->{_data} = _thaw($self->{_shm}) unless $self->{_lock};
    return (ref($self->{_data}) eq 'ARRAY') ?
	exists($self->{_data}->[$key]) : exists($self->{_data}->{$key});
}
...
sub _thaw {
	...
	#thaw failed for some reason
	if($@){
	    if($@ =~ /corrupt|binary/i){ #perhaps storable string is partial
		my $time = time();
		while(abs(time() - $time) < 5){
		    sleep 1;
		    $ice = $s->shmread;
		    eval '$water = thaw $ice;';
		    last if(!$@);
...
sub _tie {
    ...
    #fix for uninitialized shared memory and complex datatypes
    if($type ne 'SCALAR' && !defined($sh->{_data})){
	$sh->{_data} = ($type eq 'ARRAY') ? [] : {};
	defined _freeze($sh->{_shm} => $sh->{_data}) or do {
            require Carp;
	    Carp::croak "Could not write to shared memory: $!\n";
        };
    }
