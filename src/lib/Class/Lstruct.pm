...
sub struct {
    ...
    $out = "{\n  package $class;\n  use Carp;\n  sub new {\n";
    $out .= "    my \$class = (\@_) ? \$_[0] : __PACKAGE__;\n";
    $out .= "    my \%init = map {(\$_ % 2 == 0) ? \$_[\$_] : uc(\$_[\$_])} (1..\$#_);\n";

    my $cnt = 0;
    my $idx = 0;
    my( $cmt, $name, $type, $elem, $meth);
	...
	$meth = lc($name);
        ...
        if( $type eq '@' ){
            $out .= "    croak 'Initializer for $meth must be array reference'\n"; 
            $out .= "        if defined(\$init{'$name'}) && ref(\$init{'$name'}) ne 'ARRAY';\n";
            $out .= "    \$r->$meth( $init [] );$cmt\n"; 
            $arrays{$name}++;
        }
        elsif( $type eq '%' ){
            $out .= "    croak 'Initializer for $meth must be hash reference'\n";
            $out .= "        if defined(\$init{'$name'}) && ref(\$init{'$name'}) ne 'HASH';\n";
            $out .= "    \$r->$meth( $init {} );$cmt\n";
            $hashes{$name}++;
        }
        elsif ( $type eq '$') {
            $out .= "    \$r->$meth( $init undef );$cmt\n";
        }
        elsif( $type =~ /^\w+(?:::\w+)*$/ ){
            $out .= "    if (defined(\$init{'$name'})) {\n";
            $out .= "       if (ref \$init{'$name'} eq 'HASH')\n";
            $out .= "            { \$r->$meth( $type->new(\%{\$init{'$name'}}) ) } $cmt\n";
            $out .= "       elsif (UNIVERSAL::isa(\$init{'$name'}, '$type'))\n";
            $out .= "            { \$r->$meth( \$init{'$name'} ) } $cmt\n";
            $out .= "       else { croak 'Initializer for $meth must be hash or $type reference' }\n";
            $out .= "    }\n";
            $classes{$name} = $type;
            $got_class = 1;
        }
    ...
    if( $base_type eq 'HASH' ){
	$out .= "\n    Internals::SvREADONLY \%{\$r}, 1; #lock keys\n";
    }
    elsif( $base_type eq 'ARRAY' ){
	$out .= "\n    Internals::SvREADONLY \@{\$r}, 1; #lock indexes\n";
    }
    $out .= "\n    \$r;\n  }\n";
	...
	$meth = lc($name);
        if ( do { no strict 'refs'; defined &{$class . "::$meth"} } ) {
            warnings::warnif("function '$meth' already defined, overrides struct accessor method");
        }
        else {
            $pre = $pst = $cmt = $sel = $att = '';
	    ...
	    elsif( defined $lvals{$name} ){
		$att = ' : lvalue';
	    }

	    $out .= "  sub $meth$att {$cmt\n    my \$r = \$_[0];\n";
	    ...
	    $out .= "    if(\@_ == 1){ return \$r->$elem; }\n";
	    
            if( defined $arrays{$name} ){
		$sel = "->[\$_[1]]";
		$out .= "    if(\@_ == 2){\n";
		$out .= "        if(ref(\$_[1]) eq 'ARRAY'){ \$r->$elem = \$_[1]; return \$r; }\n";
		$out .= "        return $pre\$r->$elem$sel$pst;\n";
		$out .= "    }\n";
		$out .= "    if(\@_ == 3){ return ($pre\$r->$elem$sel = \$_[2]$pst); }\n";
            }
            elsif( defined $hashes{$name} ){
		$sel = "->{\$_[1]}";
		$out .= "    if(\@_ == 2){\n";
		$out .= "        if(ref(\$_[1]) eq 'HASH'){ \$r->$elem = \$_[1]; return \$r; }\n";
		$out .= "        return $pre\$r->$elem$sel$pst;\n";
		$out .= "    }\n";
		$out .= "    if(\@_ == 3){ return ($pre\$r->$elem$sel = \$_[2]$pst); }\n";
            }
            elsif( defined $classes{$name} ){
                $out .= "    croak '$meth argument is wrong class' if \@_ && ! UNIVERSAL::isa(\$_[1], '$classes{$name}');\n";
		$out .= "    if(\@_ == 2){ return ($pre\$r->$elem$sel = \$_[1]$pst); }\n";
            }
	    else{
		$out .= "    if(\@_ == 2){ return ($pre\$r->$elem$sel = \$_[1]$pst); }\n";
	    }

	    $out .= "    croak 'Too many args to $meth';\n";
            $out .= "  }\n";
        }
    }
    $out .= "}\n1;\n";
...
Class::Lstruct - declare struct-like datatypes as Perl classes
...
=head1 SYNOPSIS
    ...
    use Class::Lstruct CLASS_NAME => [ELEMENT_NAME => ELEMENT_TYPE, ...];
    use Class::Lstruct CLASS_NAME => {ELEMENT_NAME => ELEMENT_TYPE, ...};

    # declare struct at compile time, based on array, implicit
    # class name:
    package CLASS_NAME;
    use Class::Lstruct ELEMENT_NAME => ELEMENT_TYPE, ... ;
...
C<Class::Lstruct> exports a single function, C<struct>.
...
C<Class::Lstruct> can create your class at compile time.  The main reason
...
Renamed to C<Class::Lstruct> and modified by Jim Miner, 1997-04-02.
